...
public class MyDequeUsingStacks<T> {
	...
	private void move(ResizingArrayStack<T> from, ResizingArrayStack<T> to) {
		...
		if (from.size() == 1) {
			...
			//push the first half of from onto a temporary stack in reverse order
			for (int i = 0; (i < half); i++) {
				tmp.push(from.pop());
			}
			//push next half of from in reverse order onto to
			for (int i = 0; (!from.isEmpty()); i++) {
				to.push(from.pop());
			}
			//finally push every item in temp stack into from in reverse order (aka original order)
	...
	// Once you have something that works faster, you can use a higher value for
	// MIN.
